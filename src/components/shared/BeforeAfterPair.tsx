 ...
 */
export function BeforeAfterPair({ item }: { item: GalleryItem }) {
  return (
    <figure>
      <div className="grid grid-cols-2 gap-2">
        <MediaFrame src={item.beforeSrc} alt={item.beforeAlt} label="Before" />
        <MediaFrame src={item.afterSrc} alt={item.afterAlt} label="After" />
      </div>
