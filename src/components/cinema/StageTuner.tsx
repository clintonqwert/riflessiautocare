...
const SECTIONS: { title: string; note: string; controls: Control[] }[] = [
  ...
  {
    title: "Plinth & spotlight",
    note: "The platform and the downlight pooled on it.",
    controls: [
      { kind: "range", group: "stage", key: "plinthScale", label: "Light spread", min: 1, max: 6, step: 0.05,
        hint: "Keep wide so the disc edge stays out of frame" },
      { kind: "color", group: "stage", key: "plinthColor", label: "Platform colour" },
      { kind: "range", group: "stage", key: "plinthMetalness", label: "Platform metalness", min: 0, max: 1, step: 0.01,
        hint: "High kills the light pool — metals have no diffuse response" },
      { kind: "range", group: "stage", key: "plinthRoughness", label: "Platform roughness", min: 0, max: 1, step: 0.01 },
      { kind: "range", group: "stage", key: "ringWidth", label: "Bronze ring width", min: 0, max: 0.4, step: 0.01,
        hint: "0 = none. Any outline makes the floor read as a raised dais" },
      { kind: "range", group: "stage", key: "poolIntensity", label: "Light pool", min: 0, max: 2, step: 0.01,
        hint: "The visible spotlight footprint on the platform" },
      { kind: "color", group: "stage", key: "spotColor", label: "Spot colour" },
      { kind: "range", group: "stage", key: "spotIntensity", label: "Spot intensity", min: 0, max: 1500, step: 10 },
      { kind: "range", group: "stage", key: "spotHeight", label: "Spot height", min: 2, max: 16, step: 0.1 },
      { kind: "range", group: "stage", key: "spotAngle", label: "Spot angle", min: 8, max: 70, step: 1 },
      { kind: "range", group: "stage", key: "spotPenumbra", label: "Spot softness", min: 0, max: 1, step: 0.01 },
      { kind: "range", group: "stage", key: "shadowOpacity", label: "Ground shadow", min: 0, max: 1, step: 0.01 },
      { kind: "range", group: "stage", key: "shadowBlur", label: "Shadow blur", min: 0.2, max: 6, step: 0.1 },
    ],
  },
  {
    title: "Light",
