...
- **No asset downloads for the 3D scene** — geometry is procedurally generated (`car-silhouette-geometry.ts`) and lit with baked `Lightformer` planes rather than GLB/HDR assets, keeping the homepage's heaviest feature dependency-free of binary asset weight.
